    ...
    function connect() {
      ...
      socket.onmessage = ({ data }) => {
        try {
          ...
          if (msg.type === 'stockData' && msg.data) {
            // 新後端送 {symbol, name, price, changePercent, ...}
            // 轉成舊 UI 期望的 LiveStock 格式
            const d = msg.data as any;
            const isTW = d.symbol ? /\.(TW|TWO)$/i.test(d.symbol) : !isNaN(Number(d.ticker));
            const ticker = d.ticker || (d.symbol || '').replace(/\.(TW|TWO)$/i, '');
            const parseNT = (s: string | undefined) => parseFloat((s || '0').replace(/[^0-9.]/g, '')) || 0;
            const raw: LiveStock = d.ticker
              ? d  // 舊後端格式，直接用
              : {
                  ticker,
                  name:     d.name || ticker,
                  fullName: d.name || ticker,
                  market:   isTW ? 'TW' : 'US',
                  sym:      isTW ? 'NT$' : '$',
                  price:    d.price,
                  change:   d.change || 0,
